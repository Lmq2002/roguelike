...
void CoinBox::openAni()
{
	...
	Vector <RedBottle*>bottleList;
	bottleList.pushBack(RedBottle::createRedBottle(true));
	bottleList.pushBack(RedBottle::createRedBottle(false));
	int oper1 = bottleList.empty() ? -1 : bottleList.size() - 1;
	for (int i = 0; i <= oper1 && i >= 0; i++)
	{
		addChild(bottleList.at(i));
		bottleList.at(i)->bindPlayer(m_player);
		bottleList.at(i)->setPosition(coinBox->getPosition());
		bottleList.at(i)->disperseAni();
		//coinList.at(i)->flyAni(m_player->getPosition());
	}
}
